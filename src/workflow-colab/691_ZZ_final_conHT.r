...
PARAM$semillas  <- c( 211403, 311453, 411473, 511457, 611441 ) #reemplazar por las propias semillas
...
dir.create( paste0( base_dir, "exp202102/", PARAM$experimento, "/"), showWarnings= FALSE )
setwd(paste0( base_dir, "exp202102/", PARAM$experimento, "/"))   #Establezco el Working Directory DEL EXPERIMENTO

GrabarOutput()
write_yaml( PARAM, file= "parametros.yml" )   #escribo parametros utilizados

#leo la salida de la optimizaciob bayesiana
arch_log  <- paste0( base_dir, "exp202102/", PARAM$exp_input, "/BO_log.txt" )
tb_log  <- fread( arch_log )
setorder( tb_log, -ganancia )

#leo el nombre del expermento de la Training Strategy
arch_TS  <- paste0( base_dir, "exp202102/", PARAM$exp_input, "/TrainingStrategy.txt" )
TS  <- readLines( arch_TS, warn= FALSE )

#leo el dataset donde voy a entrenar el modelo final
arch_dataset  <- paste0( base_dir, "exp202102/", TS, "/dataset_train_final.csv.gz" )
dataset  <- fread( arch_dataset )

#leo el dataset donde voy a aplicar el modelo final
arch_future  <- paste0( base_dir, "exp202102/", TS, "/dataset_future.csv.gz" )
dfuture  <- fread( arch_future )

#dfuture[foto_mes != "202109", .(count = .N),  by = foto_mes]
...
for( modelo_rank in  PARAM$modelos_rank )
{
  ...
  for( vsemilla  in PARAM$semillas )
  {
    ...
    #exporto archivo con n_envios y ganancia acumulada por periodo
    setorder( tb_prediccion, -prob )
    tb_prediccion[ , n_envio := seq_len(.N), by = foto_mes]
    tb_prediccion[ , ganancia := ( ifelse(clase_ternaria== "BAJA+2", 117000, -3000)) ]
    tb_prediccion[ , ganancia_acumulada := cumsum( ganancia), by = foto_mes]
    
    nom_pred2  <- paste0( "pred_",
                          nombre_raiz,
                          "_env_gan",
                          ".csv"  )
    
    fwrite( tb_prediccion[ ,list(numero_de_cliente, foto_mes, prob, clase_ternaria, n_envio, ganancia, ganancia_acumulada)],
            file= nom_pred2,
            sep= "\t" )
    
    
    nom_pred3  <- paste0( "pred_",
                          nombre_raiz,
                          "_cortes",
                          ".csv"  )
    ...
    #exporto archivo luego de generar "cortes"
    fwrite( tb_prediccion[ n_envio %in% cortes ,list(numero_de_cliente, foto_mes, prob, clase_ternaria, n_envio, ganancia, ganancia_acumulada)],
            file= nom_pred3,
            sep= "\t" )
