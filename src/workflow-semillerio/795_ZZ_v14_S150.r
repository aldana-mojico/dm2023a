...
PARAM  <- list()
PARAM$experimento  <- "ZZ7950US005V14S150" ##cambiar V0 V1 V2 // V3 V4
PARAM$exp_input  <- "HT7510US005V14" ##cambiar V0 V1 V2 // V3 V4

PARAM$modelos_rank  <- c( 3,4 )  #Que modelos quiero, segun su posicion en el ranking e la Bayesian Optimizacion, ordenado por ganancia descendente
PARAM$semillerio  <- 150   # cantidad de semillas a utilizar en el semillerio, podrian ser 50 o 100 para mayor estabilidad
PARAM$semilla_primos  <- 211403  # se utiliza para generar el vector de  PARAM$semillerio  semillas
...
arch_log  <- paste0( PARAM$home, "expfinal/", PARAM$exp_input, "/BO_log.txt" )
tb_log  <- fread( arch_log )
setorder( tb_log, -ganancia )

#leo el nombre del expermento de la Training Strategy
arch_TS  <- paste0( PARAM$home, "expfinal/", PARAM$exp_input, "/TrainingStrategy.txt" )
TS  <- readLines( arch_TS, warn= FALSE )

#leo el dataset donde voy a entrenar el modelo final
arch_dataset  <- paste0( PARAM$home, "expfinal/", TS, "/dataset_train_final.csv.gz" )
dataset  <- fread( arch_dataset )

#leo el dataset donde voy a aplicar el modelo final
arch_future  <- paste0( PARAM$home, "expfinal/", TS, "/dataset_future.csv.gz" )
