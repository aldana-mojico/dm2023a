...
PARAM  <- list()
PARAM$experimento  <- "HT7510US005V0RF" ##cambiar V0 V0RF V1(rf) V2 // V3(rf) V4

PARAM$exp_input  <- "TS741005V0RF" ##cambiar V0 V1 V2 // V3 V4

PARAM$lgb_crossvalidation_folds  <- 5  #En caso que se haga cross validation, se usa esta cantidad de folds

PARAM$lgb_semilla  <- 211403   #cambiar por su propia semilla
...
exp_log  <- function( reg, arch=NA, folder="./expfinal/", ext=".txt", verbose=TRUE )
{
  archivo  <- arch
...
dataset_input  <- paste0( "./expfinal/", PARAM$exp_input, "/dataset_training.csv.gz" )
